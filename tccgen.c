...
static void gen_cast(CType *type)
{
    ...
    if (sbt != dbt) {
        if (sbt == VT_VOID)
            tcc_error("cannot cast from void");
...
static void gen_assign_cast(CType *dt)
{
    ...
    dbt = dt->t & VT_BTYPE;
    sbt = st->t & VT_BTYPE;
    if (((sbt == VT_VOID) || (dbt == VT_VOID)) && sbt != dbt) {
    ...
    if (tok == TOK_IF) {
        ...
        if (tok != ';') {
            ...
            if ((func_vt.t & VT_BTYPE) == VT_STRUCT) {
                CType type, ret_type;
                int ret_align, ret_sret, regsize;
                RegArgs args;

                ret_sret = gfunc_sret(&func_vt, func_var, &ret_type,
                                      &ret_align, &regsize, &args);
                if (!ret_sret) {
                    ...
                    int i;
#else
                    int ret_nregs = regargs_nregs(&args);
#endif
